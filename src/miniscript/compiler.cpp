...
Node MakeNode(Args&&... args) {
  return miniscript::MakeNodeRef<CompilerContext::Key>(
      miniscript::internal::NoDupCheck{}, std::forward<Args>(args)...);
}
...
struct Policy {
  ...
  Policy(Type nt, std::vector<CompilerContext::Key>&& key)
  ...
  Policy(Type nt, std::vector<CompilerContext::Key>&& key, uint32_t kv)
...
Policy ParseProb(Span<const char>& in, uint32_t& prob) {
  ...
  while (in.size() && in[0] >= ('0' + (prob == 0)) && in[0] <= '9') {
    prob = std::min<uint32_t>(prob * 10 + (in[0] - '0'),
...
struct Strat {
  enum class Type {
    ...
    WRAP_V,
    WRAP_J,
    WRAP_N,        // Several kinds of wrappers that don't change semantics
    ALTERNATIVES,  // Every subgraph is a separate compilation strategy; try
                   // each once
    CACHE,         // sub[0] is the dependency; sub[1] and higher are (possibly
            // self-referential) improvements to try repeatedly until all of
            // them stop improving
  };
  ...
  explicit Strat(Type nt, std::vector<unsigned char> dat)
      : node_type(nt), data(std::move(dat)) {}
  explicit Strat(Type nt, std::vector<unsigned char> dat, int64_t kv)
      : node_type(nt), data(std::move(dat)), k(kv) {}
  explicit Strat(Type nt, std::vector<const Strat*> subs)
      : node_type(nt), sub(std::move(subs)) {}
  explicit Strat(Type nt, std::vector<CompilerContext::Key> key)
      : node_type(nt), keys(std::move(key)) {}
  explicit Strat(Type nt, std::vector<const Strat*> subs, double probs)
      : node_type(nt), sub(std::move(subs)), prob(probs) {}
  explicit Strat(Type nt, std::vector<const Strat*> subs, int64_t kv,
                 double probs)
      : node_type(nt), sub(std::move(subs)), k(kv), prob(probs) {}
  explicit Strat(Type nt, std::vector<const Strat*> subs, int64_t kv)
      : node_type(nt), sub(std::move(subs)), k(kv) {}
  explicit Strat(Type nt, std::vector<CompilerContext::Key> key, int64_t kv)
      : node_type(nt), keys(std::move(key)), k(kv) {}
};
...
const Strat* ComputeStrategy(
    const Policy& node, std::unordered_map<const Policy*, const Strat*>& cache,
    StratStore& store);

const Strat* GetStrategy(const Policy& node,
...
static const Strat* STRAT_FALSE =
    MakeStrat(STRAT_GLOBAL, Strat::Type::CACHE,
              Vector(MakeStrat(STRAT_GLOBAL, Strat::Type::JUST_0)));
static const Strat* STRAT_TRUE =
    MakeStrat(STRAT_GLOBAL, Strat::Type::CACHE,
              Vector(MakeStrat(STRAT_GLOBAL, Strat::Type::JUST_1)));
...
const Strat* ComputeStrategy(
  ...
  switch (node.node_type) {
    ...
    case Policy::Type::AND: {
      ...
      strats.push_back(
          MakeStrat(store, Strat::Type::AND, Vector(left, right)));  // and(X,Y)
      strats.push_back(
          MakeStrat(store, Strat::Type::ANDOR,
                    Vector(std::move(left), std::move(right), STRAT_FALSE),
                    1.0));  // or(and(X,Y),0)
      break;
    }
    case Policy::Type::OR: {
      ...
      const auto right = GetStrategy(node.sub[1], cache, store);
      if (!left || !right) return {};
      if (node.sub[0].node_type == Policy::Type::AND &&
          node.sub[0].sub.size() == 2) {
        ...
        strats.push_back(MakeStrat(
            store, Strat::Type::ANDOR,
            Vector(std::move(leftleft), std::move(leftright), right), prob));
      }
      if (node.sub[1].node_type == Policy::Type::AND &&
          node.sub[1].sub.size() == 2) {
        ...
        strats.push_back(
            MakeStrat(store, Strat::Type::ANDOR,
                      Vector(std::move(rightleft), std::move(rightright), left),
                      1.0 - prob));
      }
      strats.push_back(MakeStrat(store, Strat::Type::ANDOR,
                                 Vector(left, STRAT_TRUE, right), prob));
      strats.push_back(MakeStrat(store, Strat::Type::OR,
                                 Vector(std::move(left), std::move(right)),
                                 prob));
      break;
    }
    case Policy::Type::THRESH: {
      ...
      std::transform(
          node.sub.begin(), node.sub.end(), std::back_inserter(subs),
          [&](const Policy& x) { return GetStrategy(x, cache, store); });
      for (const auto& s : subs) {
        if (!s) return {};
      }
      if (node.sub.size() <= 20 &&
          std::all_of(node.sub.begin(), node.sub.end(), [&](const Policy& x) {
            return x.node_type == Policy::Type::PK_K;
          })) {
        std::vector<CompilerContext::Key> keys;
        ...
        strats.push_back(
            MakeStrat(store, Strat::Type::MULTI, std::move(keys), node.k));
      }
      if (node.k == 1 || node.k == node.sub.size()) {
        while (subs.size() > 1) {
          auto rep = MakeStrat(
              store, node.k == 1 ? Strat::Type::OR : Strat::Type::AND,
              Vector(*(subs.rbegin() + 1), subs.back()), 1.0 / subs.size());
          subs.pop_back();
          subs.pop_back();
      ...
      strats.push_back(MakeStrat(store, Strat::Type::THRESH, subs, node.k,
                                 (double)node.k / subs.size()));
      break;
    }
  }
  ...
  if (strats.size() != 1) {
    ...
    strats.push_back(
        MakeStrat(store, Strat::Type::ALTERNATIVES, std::move(sub)));
  }

  auto ret = MakeMutStrat(store, Strat::Type::CACHE, std::move(strats));
  ret->sub.push_back(
      MakeStrat(store, Strat::Type::WRAP_C, std::vector<const Strat*>{ret}));
  ret->sub.push_back(
      MakeStrat(store, Strat::Type::WRAP_V, std::vector<const Strat*>{ret}));
  ret->sub.push_back(MakeStrat(store, Strat::Type::AND,
                               std::vector<const Strat*>{ret, STRAT_TRUE}));
  ret->sub.push_back(
      MakeStrat(store, Strat::Type::WRAP_N, std::vector<const Strat*>{ret}));
  ret->sub.push_back(
      MakeStrat(store, Strat::Type::WRAP_D, std::vector<const Strat*>{ret}));
  ret->sub.push_back(
      MakeStrat(store, Strat::Type::WRAP_J, std::vector<const Strat*>{ret}));
  ret->sub.push_back(MakeStrat(store, Strat::Type::OR,
                               std::vector<const Strat*>{ret, STRAT_FALSE},
                               1.0));
  ret->sub.push_back(
      MakeStrat(store, Strat::Type::WRAP_AS, std::vector<const Strat*>{ret}));
...
struct Result {
  ...
  Result(Node&& in_node, const CostPair& in_pair, double in_cost)
      : node(std::move(in_node)), pair(in_pair), cost(in_cost) {}
};
...
typedef std::pair<miniscript::Type, miniscript::Type>
    TypeFilter;  // First element is required type properties; second one is
                 // which one we care about

constexpr TypeFilter ParseFilter(const char* c, size_t len, size_t split) {
  return c[split] == '/'
             ? TypeFilter{operator"" _mst(c, split), operator"" _mst(
                                                         c + split + 1,
                                                         len - split - 1)}
         : split == len ? TypeFilter{operator"" _mst(c, len), ""_mst}
                        : ParseFilter(c, len, split + 1);
}

constexpr TypeFilter operator"" _mstf(const char* c, size_t len) {
  return ParseFilter(c, len, 0);
}
...
struct Compilation {
  ...
  void Add(const CostPair& pair, Node node) {
    /* We cannot use IsSaneSubexpression(), as it includes CheckDuplicates which
     * we don't evaluate for interim expressions here. */
    if (!(node->ValidSatisfactions() && node->IsNonMalleable() &&
          node->CheckTimeLocksMix()))
      return;
    ...
    for (const Result& x : results) {
      ...
      if (old_typ << new_typ && (x.Compare(cost, node) <= 0))
        return;  // There is an existing element that's a subtype and better.
                 // New item is not useful.
    }
    ...
    results.erase(
        std::remove_if(results.begin(), results.end(),
                       [&](const Result& x) {
                         auto old_typ = x.node->GetType();
                         return (new_typ << old_typ &&
                                 (x.Compare(cost, node) >=
                                  0));  // Delete existing types which are
                                        // supertypes of the new type and worse.
                       }),
        results.end());
    // Add the new item.
  ...
  template <typename... X>
  void Add(const CostPair& pair, X&&... args) {
    Add(pair, MakeNode(std::forward<X>(args)...));
  }
  ...
  std::vector<Result> Query(const TypeFilter typ) const {
    ...
    for (const Result& x : results) {
      if (x.node->GetType() << typ.first) {
        ...
        if (!r.second &&
            x.Compare(r.first->second.cost, r.first->second.node) < 0)
          r.first->second = x;
      }
    }
...
const Compilation& GetCompilation(const Strat* strat, double p, double q,
                                  std::map<CompilationKey, Compilation>& cache);
void Compile(const Strat* strat, Compilation& compilation,
             std::map<CompilationKey, Compilation>& cache);

constexpr double INF = std::numeric_limits<double>::infinity();

CostPair CalcCostPair(Fragment nt, const std::vector<const Result*>& s,
                      double l) {
  double r = 1.0 - l;
  if (nt != Fragment::OR_B && nt != Fragment::OR_D && nt != Fragment::OR_C &&
      nt != Fragment::OR_I && nt != Fragment::THRESH && nt != Fragment::ANDOR &&
      nt != Fragment::MULTI) {
    assert(l == 0);
  }
  switch (nt) {
    ...
    case Fragment::WRAP_D:
      return {2 + s[0]->pair.sat, 1};
    case Fragment::WRAP_V:
      return {s[0]->pair.sat, INF};
    case Fragment::WRAP_J:
      return {s[0]->pair.sat, 1};
    case Fragment::JUST_1:
      return {0, INF};
    case Fragment::JUST_0:
      return {INF, 0};
    case Fragment::AND_V:
      return {s[0]->pair.sat + s[1]->pair.sat, INF};
    case Fragment::AND_B:
      return {s[0]->pair.sat + s[1]->pair.sat,
              s[0]->pair.nsat + s[1]->pair.nsat};
    case Fragment::OR_B:
      return {Mul(l, s[0]->pair.sat + s[1]->pair.nsat) +
                  Mul(r, s[0]->pair.nsat + s[1]->pair.sat),
              s[0]->pair.nsat + s[1]->pair.nsat};
    case Fragment::OR_D:
    case Fragment::OR_C:
      return {Mul(l, s[0]->pair.sat) + Mul(r, s[0]->pair.nsat + s[1]->pair.sat),
              s[0]->pair.nsat + s[1]->pair.nsat};
    case Fragment::OR_I:
      return {Mul(l, s[0]->pair.sat + 2) + Mul(r, s[1]->pair.sat + 1),
              std::min(2 + s[0]->pair.nsat, 1 + s[1]->pair.nsat)};
    case Fragment::ANDOR:
      return {Mul(l, s[0]->pair.sat + s[1]->pair.sat) +
                  Mul(r, s[0]->pair.nsat + s[2]->pair.sat),
              s[0]->pair.nsat + s[2]->pair.nsat};
    case Fragment::MULTI:
      return CostPair{1.0 + l * 73.0, 1.0 + l};
...
std::pair<std::vector<double>, std::vector<double>> GetPQs(Fragment nt,
                                                           double p, double q,
                                                           double l, int m) {
  ...
  switch (nt) {
    ...
    case Fragment::AND_B:
      return {{p, p}, {q, q}};
    case Fragment::OR_B:
      return {{l * p, r * p}, {r * p + q, l * p + q}};
    case Fragment::OR_D:
      return {{l * p, r * p}, {r * p + q, q}};
    case Fragment::OR_C:
      return {{l * p, r * p}, {r * p, 0}};
    case Fragment::OR_I:
      return {{l * p, r * p}, {m == 0 ? q : 0, m == 1 ? q : 0}};
    case Fragment::ANDOR:
      return {{l * p, l * p, r * p}, {q + r * p, 0, q}};
    case Fragment::THRESH:
      return {std::vector<double>(m, p * l), std::vector<double>(m, q + p * r)};
  }
  assert(false);
  return NONE;
}
...
const TypeFilters& GetTypeFilter(Fragment nt) {
  ...
  static const TypeFilters FILTER_AND_V{{"V/nzoms"_mstf, "B/unzofmsx"_mstf},
  ...
  static const TypeFilters FILTER_OR_I{{"V/zudfems"_mstf, "V/zudfems"_mstf},
  ...
  switch (nt) {
    ...
    case Fragment::RIPEMD160:
      return FILTER_NO;
    case Fragment::WRAP_A:
      return FILTER_WRAP_A;
    case Fragment::WRAP_S:
      return FILTER_WRAP_S;
    case Fragment::WRAP_C:
      return FILTER_WRAP_C;
    case Fragment::WRAP_D:
      return FILTER_WRAP_D;
    case Fragment::WRAP_V:
      return FILTER_WRAP_V;
    case Fragment::WRAP_J:
      return FILTER_WRAP_J;
    case Fragment::WRAP_N:
      return FILTER_WRAP_N;
    case Fragment::AND_V:
      return FILTER_AND_V;
    case Fragment::AND_B:
      return FILTER_AND_B;
...
template <typename... Args>
void AddInner(Compilation& compilation,
              std::map<CompilationKey, Compilation>& cache, Fragment nt,
              const std::vector<const Result*>& resp, double prob,
              Args&&... args) {
  std::vector<Node> subs;
  for (const Result* res : resp) subs.push_back(res->node);
  compilation.Add(CalcCostPair(nt, resp, prob), nt, std::move(subs),
                  std::forward<Args>(args)...);
}

template <typename... Args>
void Add(Compilation& compilation, std::map<CompilationKey, Compilation>& cache,
         Fragment nt, const std::vector<const Strat*>& s, double prob, int m,
         Args&&... args) {
  ...
  for (size_t j = 0; j < filter.size(); ++j) {
    ...
    for (size_t i = 0; i < s.size(); ++i) {
      const Compilation& subcomp =
          GetCompilation(s[i], pqs.first[i], pqs.second[i], cache);
    ...
    for (uint32_t comb = 0; comb < num_comb; ++comb) {
      ...
      if (comb + 1 == num_comb) {
        AddInner(compilation, cache, nt, resp, prob,
                 std::forward<Args>(args)...);
      } else {
...
const Compilation& GetCompilation(
    const Strat* strat, double p, double q,
    std::map<CompilationKey, Compilation>& cache) {
  ...
  switch (strat->node_type) {
    ...
    case Strat::Type::OLDER: {
      Add(compilation, cache,
          strat->node_type == Strat::Type::OLDER ? Fragment::OLDER
                                                 : Fragment::AFTER,
          strat->sub, 0, 0, strat->k);
      return;
    }
    ...
    case Strat::Type::RIPEMD160: {
      Add(compilation, cache, Fragment::RIPEMD160, strat->sub, 0, 0,
          strat->data);
      return;
    }
    ...
    case Strat::Type::MULTI:
      Add(compilation, cache, Fragment::MULTI, strat->sub, strat->k, 0,
          strat->keys, strat->k);
      return;
    ...
    case Strat::Type::THRESH: {
      auto pqs =
          GetPQs(Fragment::THRESH, p, q, strat->prob, (int)strat->sub.size());
      std::vector<Result> Bs, Ws;
      int B_pos = -1;
      double cost_diff = -1.0;
      for (size_t i = 0; i < strat->sub.size(); ++i) {
        const Compilation& comp =
            GetCompilation(strat->sub[i], pqs.first[i], pqs.second[i], cache);
      ...
      AddInner(compilation, cache, Fragment::THRESH, resp, strat->prob,
               strat->k);
      return;
    }
  }
...
std::string Disassembler(CScript::const_iterator& it,
                         CScript::const_iterator end, int indent = 0) {
  std::string ret;
  bool newline = true;
  ...
  while (it != end) {
    ...
    if (data.size() == 20) {
      if (data == std::vector<unsigned char>(20, 0x99)) {
        ...
        ret += "<HASH160(" +
               std::string((const char*)data.data() + 3, data.size() - 3) +
               ")>";
      }
    } else if (data.size() == 32 &&
               data == std::vector<unsigned char>(32, 0x88)) {
      ret += "<H>";
    } else if (data.size() == 33 && data[0] == 2 && data[1] == 'P' &&
               data[2] == 'K' && data[3] == 'b') {
      while (data.size() && data.back() == 0) data.pop_back();
...
std::string DebugNode(const Node& node) {
    switch (node->fragment) {
        ...
        case Fragment::MULTI: return "multi(" + std::to_string(node->k) + " of "
+ std::to_string(node->keys.size()) + ")"; case Fragment::AFTER: return "after";
        ...
        case Fragment::THRESH: return "thresh(" + std::to_string(node->k) + " of
" + std::to_string(node->subs.size()) + ")";
    }
    assert(false);
    return "";
}
...
DebugNode(res.node).c_str(), res.p, res.q, (int)res.node->ScriptSize(),
res.pair.sat, res.pair.nsat, res.cost); assert(res.subs.size() ==
res.node->subs.size()); for (size_t j = 0; j < res.subs.size(); ++j) {
...
bool Compile(const std::string& policy,
             miniscript::NodeRef<CompilerContext::Key>& ret, double& avgcost) {
