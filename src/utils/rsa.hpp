...
inline std::string EnvelopeSeal(const std::string& pub_key, const std::string& plain) {
    ...
    unsigned char *encrypted_key=(unsigned char*)malloc(EVP_PKEY_size(evp_pbkey));
    ...
    EVP_SealInit(ctx, EVP_aes_256_cbc(), &encrypted_key,&encrypted_key_len, iv.data(), &evp_pbkey, 1);
...
inline std::string EnvelopeOpen(const std::string& pub_key, const std::string& priv_key, const std::string& cyphered) {
    ...
    ctx = EVP_CIPHER_CTX_new();
    EVP_OpenInit(ctx, EVP_aes_256_cbc(), encrypted_key.data(),encrypted_key.size(), iv.data(), evp_prkey);

    std::vector<unsigned char> plaintext(ciphertext.size());
    EVP_OpenUpdate(ctx, &plaintext[0], &len, ciphertext.data(), ciphertext.size());
    plaintext_len = len;
